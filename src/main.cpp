...
int main(int argc, char* argv[])
{
    ...
    for (const QString& locale : std::as_const(uiLanguages)) {
        const QString baseName = "comma-qt_"_L1 + QLocale(locale).name();
    ...
    parser.setApplicationDescription(QCoreApplication::translate("main", "%1 - Easy-to-use application for viewing and editing tabular files").arg(app.applicationName()));
